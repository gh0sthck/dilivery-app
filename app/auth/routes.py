...
async def get_current_user(creds: HTTPAuthorizationCredentials = Depends(http_bearer)) -> UserSchema:
    token: dict = decode_jwt(jwt_token=creds.credentials.encode())
    current_user = await users_explorer.get_by_username(
...
async def user_current_user(schema: UserSchema = Depends(get_current_user)) -> UserSchema: 
    return schema
